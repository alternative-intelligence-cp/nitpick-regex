...
- [x] the pin's sums and ancestry — `5bdae98` an ancestor, `2dde296` not (the fifth audit's item 1, restated); the red run at `c970483` recorded (`78/223`, `vec.npk`'s two `NITPICK-TYPE-047`s); `214/214` GREEN at `c3bdae2`  
      **DONE** — blocks 0a–0c `SAME`: both pins' sums `OK`, `c3bdae2` and `5bdae98` ancestors of `c970483` and `2dde296` not, the two differing by DEF-104's one file (16+, 5−); `78/223 unit(s) passed in 56.3 s.`, `harness exit 1`, `vec.npk` refused `TYPE-047` at 487:5 and 533:5; `214/214` in 93.2 s at `c3bdae2`, GREEN.
- [x] `vec_pop` spells `move(...)`; `Pod` with nine scalar impls, re-exported from `core.npk`; `vec_get<T: Pod>`; `src/` compiling at both pins; bills and sizes unchanged  
      **DONE** — `06d5c00`, RX-168; all thirteen `src/` files compile at `c970483` and at `c3bdae2` (`codes` over `git ls-files 'src/*.npk'`, 13 of 13 twice); the nine bills identical to `c3bdae2`'s (`diff` exit 0: 10, 10, 10, 10, 6, 11, 6, 6, 6); sizes 152 and 32; the floor 5 / 6 / `{npk_sys6}`.
- [x] the six loan and pass-out pins and probe 17 refused exactly their codes at measured positions and moved under their names; `vec_owning_get_moves_out` refused `NITPICK-TYPE-017`; each refusal shown to be the pin's or the bound's  
      **DONE** — RX-169; nine `git mv`s; `TYPE-085` at 35:5 (probe 17), 25:25, 22:20, 32:32, 32:28, 24:23, `TYPE-047` at 27:5, `TYPE-017` at 32:31, one diagnostic each, pinned by measurement; at `c3bdae2` the seven still run 0, 95, 0, 95, 0, 0, 70 at both levels (the pin's), and the one-read fixture runs 0 / 0 against the tree before (the bound's). **N-26's `for` binding is refused**, so the gate's loan clause holds for it.
- [x] `loan_spellings` runs; probe 15 exits 4; probe 18 exits 95; `lexical.py` closes at `"""` and case 18 reads the two closes apart; `c3bdae2`'s close fails case 18 alone  
      **DONE** — RX-170; `loan_spellings` 0 / 0 and probe 18 95 / 95 at both pins, probe 15 4 / 4 at `c970483` and refused `LEX-005 PARSE-001 PARSE-003` at `c3bdae2`; with the old close put back in a copy the self-check exits 1, case 18 its one `FAIL`, the substitution counted (the new close ×1, then ×0). Probe 18's defect is the workbench registry's O-N28, confirmed after this dispatch as the compiler's DEF-116 (its landing 69, `TYPE-014` on a `move` mismatch).
- [x] DEF-95, DEF-96, DEF-97, DEF-99/105/106, DEF-103 and DEF-108 each measured: nothing to drop, nothing refused, N-21's shapes building, no `FLOW-001` site  
      **DONE** — no file names `BadStep`; the sweeps hold no `TYPE-083`, `TYPE-084`, `TYPE-086` or `PARSE-001`, and a two-parameter `main` is `TYPE-083` at `c970483`; N-21's four shapes `llc`-refused at `c3bdae2` and 0 / 0 at `c970483`; zero `FLOW-001` in the 89 files the compiler flow-checks, and every function in the 28 it cannot — 66 read by the block and a 67th, an `impl` method the block's reader cannot see, read by eye — ends in `pass`, `exit` or `fail`.
- [x] PD-12 … PD-14 recorded; the markers; O-N17 re-homed, O-N21 and O-N22 discharged, the O-R entry opened; probe 06b's and 07's headers dated  
      **DONE** — RX-168, RX-169, RX-170, each with its alternatives declined; five `SUPERSEDED IN PART` markers; O-N17 struck for O-N27 and O-N27's local entry; O-N21 and O-N22 struck as discharged; O-R3 opened; and, the dispatch having numbered §6.1's defect, its local O-N28 entry — the docs script's 57th edit.
- [x] every sweep's per-file movement against the plan's §7; `218/218` GREEN at `c970483`; `check_refs` clean before and after staging; CI green on both commits, read from the log, `npkc.ll` compared with notice 66's  
      **DONE** — 63 rows of 63 equal to §7's, regenerated by script; `218/218` in 92.1 s over the staged tree and again over `06d5c00` as committed, GREEN; `check_refs` clean before (73, leak scan 230) and after staging (73, 232); CI run `36231854475` green on `06d5c00`, read from job `108376379756`'s log through the API — `compiler pin: c9704830…`, `218/218`, `GREEN.`, `rx120: every asserted leg held.`, `20 live, 4 pending`, `1 nested repository pruned: .nitpick`, the consumer `exit 0` — and its `npkc.ll` row, 28 188 736 bytes and `d36a7e23…fe3183`, EQUAL to notice 66's to 64 hex and the byte, a one-digit-off control unequal. The record commit's own run is read after its push, since a commit cannot name its own CI.
