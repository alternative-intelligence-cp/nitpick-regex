#!/usr/bin/env python3
"""0.0.4e -- run the plan's command blocks, and COMPARE each one's output with the
plan's expected output. `PLAYBOOK.md` §12: "the durable form of 'quote a command's
whole output' is a COMPARATOR" -- care drops lines; this does not.

    python3 e4e_blocks.py list    "$REPO"             # the blocks, in order
    python3 e4e_blocks.py run     "$REPO" <id> [...]  # run blocks, save outputs
    python3 e4e_blocks.py compare "$REPO" <id> [...]  # diff saved outputs vs Expect

A BLOCK is a fenced `bash` block in `0.0.4e.md` whose first line is `# block <id>`;
...
`$REPO/.internal/d4e/blocks/<id>.out`.
...
PLAN = os.path.join("meta", "roadmap", "0.0", "0.0.4e.md")
...
def blocks(root):
    text = open(os.path.join(root, PLAN), encoding="utf-8").read()
    runs, expects = {}, {}
    order = []
...
def main(argv):
    ...
    ids = argv[3:] or order
    outdir = os.path.join(root, ".internal", "d4e", "blocks")
